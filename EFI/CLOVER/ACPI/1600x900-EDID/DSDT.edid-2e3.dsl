 ...
 *
 * Disassembly of dsdt.aml, Tue Jan  5 08:25:46 2016
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x0000AF57 (44887)
 *     Revision         0x01 **** 32-bit table (V1), no 64-bit math support
 *     Checksum         0x44
 *     OEM ID           "Apple "
 *     OEM Table ID     "TP-83   "
 *     OEM Revision     0x00001460 (5216)
 *     Compiler ID      "INTL"
 *     Compiler Version 0x20150515 (538248469)
 */
DefinitionBlock ("DSDT.edid-2e3.aml", "DSDT", 1, "Apple ", "TP-83   ", 0x00001460)
{
                    ...
                    EID2), LEqual (IID3, EID3))))
                {
                    ...
                    If (LNot (And (CDW1, One)))
                    {
                        If (And (CTRL, One))
                        {
                            ...
                            Store (Zero, ^EXP5.HPCE) /* \_SB_.PCI0.EXP5.HPCE */
                            Store (One, ^EXP5.HPCS) /* \_SB_.PCI0.EXP5.HPCS */
                            Store (One, ^EXP5.PDC) /* \_SB_.PCI0.EXP5.PDC_ */
                            Store (One, NHPS) /* \NHPS */
                        }
...
// tjl-begin fixes for HDMI audio
            Device (HDMI)
            {
...
// tjl-begin various IPGU graphics device 
// tjl-fix change name of VID to IGPU
// tjl-comment add DualLink 1 for 1600x900
// tjl-comment add EDID 1600x900 fix for glitch/hang when EDID is broken
            Device (IGPU)
            {
...
// tjl-begin uncomment this section for use with high-res LCD 1600x900
                          ...
                          {     
                              ...
                              0x30, 0xE4, 0xE3, 0x02, 0x00, 0x00, 0x00, 0x00,
                              0xFF, 0x14, 0x01, 0x04, 0x80, 0x1F, 0x11, 0x78,
                              0xE2, 0x33, 0x85, 0x99, 0x56, 0x55, 0x91, 0x27,
                              0x17, 0x50, 0x54, 0x00, 0x00, 0x00, 0x01, 0x01,
                              0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
                              0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0xAE, 0x24,
                              0x40, 0x5A, 0x60, 0x84, 0x1A, 0x30, 0x30, 0x20,
                              0x35, 0x00, 0x36, 0xAE, 0x10, 0x00, 0x00, 0x1A,
                              0x40, 0x1F, 0x40, 0x80, 0x60, 0x84, 0x1A, 0x30,
                              0x30, 0x20, 0x35, 0x00, 0x36, 0xAE, 0x10, 0x00,
                              ...
                              0x30, 0x0A, 0x20, 0x20, 0x20, 0x20, 0x00, 0xC6
                          },
// tjl-end
                            "hda-gfx", 
                            Buffer (0x0A)
...
// tjl-fix comment out IRQ value for this device
//                          IRQNoFlags () {2}
...
// tjl-begin - add monitor function for Temperature and Fan Speed
                Device (SMCD)
                {
...
// tjl-fix 8-bit        HFN1,   16
                    ...
                    Method (_Q16, 0, NotSerialized)  // _Qxx: EC Query
                    {
                        ...
                        Else
                        {
                            ...
                            {
                                Notify (IGPU, 0x80) // Status Change
// tjl-fix error comment out this line                               Return (Zero)
                            }
                    ...
                    Method (_Q26, 0, NotSerialized)  // _Qxx: EC Query
                    {
                        ...
                        If (^^^PEG.IGPU.ISOP ())
                        {
                            Notify (^^^PEG.IGPU, 0xDF) // Hardware-Specific
                        }
                    ...
                    Method (_Q4D, 0, NotSerialized)  // _Qxx: EC Query
                    {
                        ...
                        Else
                        {
                            If (LEqual (SLUL, One))
                            {
                                Store (Zero, ^BAT1.XB1S) /* \_SB_.PCI0.LPC_.EC__.BAT1.XB1S */
                                Notify (BAT1, 0x03) // Eject Request
                            }
                        }
...
// tjl-begin 8-bit fixes - convert variables into 8-bit entities
                    Field (ECOR, ByteAcc, NoLock, Preserve)
                    {
                        Offset (0xA0), 
                        SBR1,   8, 
...
// tjl-end
                    ...
                    Method (GBIF, 3, NotSerialized)
                    {
                        ...
                        If (Arg2)
                        {
                            ...
                            If (Local7)
                            {
                                Multiply (B1B2 (SBF1, SBF2), 0x0A, Local1)
                            }
                            Else
                            {
                                Store (B1B2 (SBF1, SBF2), Local1)
                            }
                            ...
                            If (Local7)
                            {
                                Multiply (B1B2 (SDC1, SDC2), 0x0A, Local0)
                            }
                            Else
                            {
                                Store (B1B2 (SDC1, SDC2), Local0)
                            }
                            ...
                            Else
                            {
                                If (B1B2 (SDV1, SDV2))
                                {
                                    Divide (0x00030D40, B1B2 (SDV1, SDV2), Local2, Index (Arg1, 0x06))
                                }
                                Else
                                {
                            ...
                            Store (B1B2 (SDV1, SDV2), Index (Arg1, 0x04))
                            Store (B1B2 (SSN1, SSN2), Local0)
...
// tjl-end
                    ...
                    Method (GBST, 4, NotSerialized)
                    {
                        ...
                        {
                            Store (Arg0, HIID) /* \_SB_.PCI0.LPC_.EC__.HIID */
                            Store (B1B2 (SBV1, SBV2), Local3)
                            If (Arg2)
                            {
                                Multiply (B1B2 (SBR1, SBR2), 0x0A, Local2)
                            }
                            Else
                            {
                                Store (B1B2 (SBR1, SBR2), Local2)
                            }

                            Store (B1B2 (SBA1, SBA2), Local1)
                    ...
                    Device (BAT1)
                    {
                        ...
                        Method (_STA, 0, NotSerialized)  // _STA: Status
                        {
                            If (H8DR)
                            {
                                If (HB1A)
                                {
                                    Store (One, B1ST) /* \_SB_.PCI0.LPC_.EC__.BAT1.B1ST */
                                    If (SLUL)
                                    {
                                        Store (One, SBLI) /* \_SB_.PCI0.LPC_.EC__.BAT1.SBLI */
                                        Store (Zero, XB1S) /* \_SB_.PCI0.LPC_.EC__.BAT1.XB1S */
                                    }
                                    Else
                                    {
                                        Store (Zero, SBLI) /* \_SB_.PCI0.LPC_.EC__.BAT1.SBLI */
                                        Store (One, XB1S) /* \_SB_.PCI0.LPC_.EC__.BAT1.XB1S */
                                    }
                                }
                                Else
                                {
...
// tjl-fix - remove PRW that clover added 
                    }
                    ...
                    Method (_PS0, 0, NotSerialized)  // _PS0: Power State 0
                    {
                        If (ISOP ())
                        {
                            If (DGOS)
                            {
                                ...
                                VHYB (0x04, Zero)
                                SWTT (One)
                                Store (Zero, DGOS) /* \_SB_.PCI0.PEG_.IGPU.DGOS */
                            }
                            Else
                            {
                    ...
                    Method (_PS3, 0, NotSerialized)  // _PS3: Power State 3
                    {
                        If (ISOP ())
                        {
                            If (LEqual (OMPR, 0x03))
                            {
                                ...
                                Sleep (0x64)
                                VHYB (Zero, Zero)
                                Store (One, DGOS) /* \_SB_.PCI0.PEG_.IGPU.DGOS */
                                Store (0x02, OMPR) /* \_SB_.PCI0.PEG_.IGPU.OMPR */
                            }
                        }
                    }
...
// tjl-fix change _IRC to IRC name
                    Name (IRC, Zero)
                    ...
                    Method (GETB, 3, Serialized)
                    {
                        ...
                        Return (TBF3) /* \_SB_.PCI0.PEG_.IGPU.GETB.TBF3 */
                    }
...
// tjl-fix - remove PRW
                ...
                Name (_SUN, 0x00190000)  // _SUN: Slot User Number
            }
...
// tjl-begin - replace SLOT Device with XHCI Device for SuperSpeed USB3 Host Controller
                Device (XHC1)
                {
                    Name (_ADR, Zero)  // _ADR: Address
...
// tjl-end
            ...
            Device (EXP5)
            {
                ...
                Method (_INI, 0, NotSerialized)  // _INI: Initialize
                {
                    Store (PDS, PDSF) /* \_SB_.PCI0.EXP5.PDSF */
                }
...
// tjl-begin - replace Q14 and Q15 to change brightness
...
//
//                \UCMS (0x03)
// ThinkVantage Button toggles fan speed from Auto to Max speed
                Name (FANS, One)
...
// tjl-end
        ...
        Scope (PCI0.PEG.IGPU)
        {
            Method (GOBT, 1, NotSerialized)
            {
                ...
                {
                    Return (OPVK) /* \_SB_.PCI0.PEG_.IGPU.GOBT.OPVK */
                }
        ...
        Scope (PCI0.PEG.IGPU)
        {
            ...
            Method (NVOP, 4, NotSerialized)
            {
                ...
                Store (ToInteger (Arg2), T_0) /* \_SB_.PCI0.PEG_.IGPU.NVOP.T_0_ */
                If (LEqual (T_0, Zero))
                {
                    Store (Buffer (0x04)
                        {
                ...
                Else
                {
                    If (LEqual (T_0, 0x1A))
                    {
                        ...
                        If (ToInteger (FLCH))
                        {
                            Store (OPCE, OMPR) /* \_SB_.PCI0.PEG_.IGPU.OMPR */
                        }
                        ...
                        Store (One, OPEN) /* \_SB_.PCI0.PEG_.IGPU.NVOP.OPEN */
                        Store (One, SHPC) /* \_SB_.PCI0.PEG_.IGPU.NVOP.SHPC */
                        Store (One, DGPC) /* \_SB_.PCI0.PEG_.IGPU.NVOP.DGPC */
                        If (_STA ())
                        {
                            Store (0x03, CGCS) /* \_SB_.PCI0.PEG_.IGPU.NVOP.CGCS */
                        }

                        Store (0x02, HDAC) /* \_SB_.PCI0.PEG_.IGPU.NVOP.HDAC */
                        Return (Local0)
                    }
                    Else
                    {
                        If (LEqual (T_0, 0x1B))
                        {
                            ...
                            Store (One, PWST) /* \_SB_.PCI0.PEG_.IGPU.NVOP.PWST */
                            If (LAnd (^^^LPC.EC.DOCD, ^^^LPC.EC.ADP1._PSR ()))
                            {
                                Store (One, RQGS) /* \_SB_.PCI0.PEG_.IGPU.NVOP.RQGS */
                            }
                            Else
                            {
                                Store (Zero, RQGS) /* \_SB_.PCI0.PEG_.IGPU.NVOP.RQGS */
                            }

                            If (ToInteger (HDAR))
                            {
                                Store (ToInteger (HDAU), HDAS) /* \_SB_.PCI0.PEG_.IGPU.HDAS */
                            }
...
// tjl-begin -- remove TPM.CMOR functions
        Store (One, Local0)
        If (LEqual (Arg0, SPS))
        {
            Store (Zero, Local0)
...
// tjl-comment  this section is for mode 1
            If (LEqual (Arg0, One))
            {
...
// tjl-comment  this section is for mode Sleep
            If (LEqual (Arg0, 0x03))
            {
                VVPD (0x03)
                TRAP ()
...
// tjl-comment this section is for mode Hibernate
            If (LEqual (Arg0, 0x04))
            {
                \_SB.SLPB._PSW (Zero)
                TRAP ()
...
// tjl-comment this section is for mode Shutdown
            If (LEqual (Arg0, 0x05))
            {
                TRAP ()
                AWON (0x05)
...
// tjl-begin - set system status indicators as appropriate
            If (LEqual (Arg0, 0x05))
            {
                \_SI._SST (Zero)
            }
...
// tjl-fix check for arg0 value after sleep - this should not happen - but if not 0-5, then set to 4 (LID wake)
...
// tjl-fix - add LID notification do full wake
...
// tjl-fix added call to SST to reset LEDs after sleep -- value of 1 is normal
    ...
    Scope (_GPE)
    {
        ...
        {
            Store (B1B2 (\_SB.PCI0.LPC.EC.HWK1, \_SB.PCI0.LPC.EC.HWK2), Local0)
        ...
        Method (_L16, 0, NotSerialized)  // _Lxx: Level-Triggered GPE
        {
            If (\_SB.PCI0.PEG.IGPU.ISOP ())
            {
                ...
                {
                    Notify (\_SB.PCI0.PEG.IGPU, 0x81) // Information Change
                }
            }
