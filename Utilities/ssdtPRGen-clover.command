...
# mods by tluck for T420 on Clover
#
# Updates:
...
#
# tluck set ACPI to ESP
gDestinationPath="/Volumes/ESP/EFI/CLOVER/ACPI/patched/"
...
#          sudo curl -o /usr/local/bin/iasl https://raw.githubusercontent.com/Piker-Alpha/RevoBoot/clang/i386/libsaio/acpi/Tools/iasl
	  ...
	  locald=${0%/*}
          printf "Copying iasl from $locald to /usr/local/bin\n"
	  sudo cp "$locald/iasl" /usr/local/bin
...
function _setDestinationPath
{
  ...
  #
#tluck - mods for ESP or EFI mount point
  if [[ ! -d /Volumes/ESP && ! -d /Volumes/EFI ]];
    then
      #
      # Not there.
      #
      echo 'Creating temporarily mount point: /Volumes/ESP'
...
#  location="OEM/"
#  productname=`ioreg -n FakeSMCKeyStore |grep product-name |sed -e's/^.*= //' -e's/"//g' `
#  if [[ $productname == "" ]] ; then
#       echo OEM Product not found - using generic location
#       location=""
#  fi

  if [ -d /Volumes/EFI ];
    then
          gDestinationPath=/Volumes/EFI/EFI/CLOVER/ACPI/patched/
          _debugPrint "ACPI target directory changed to: ${gDestinationPath}\n"
  fi
  if [ -d /Volumes/ESP ];
    then
  ...
  if [ $gCallIasl -eq 1 ];
    then
      ...
      if [ $gAutoCopy -eq 1 ];
		...
		if [[ $gUnmountEFIPartition == 1 ]];
                then
			printf "Unmounting ESP partition...\n"
